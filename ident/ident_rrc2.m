 function [impedance]=ident_rrc2(datetime,U,I,dod_ah,config,phases,options)
...
% hold on
% grid on
% plot(datetime_phase-datetime_phase(1),voltage_phase)
...
% [Rsid, R1id, C1id, R2id, C2id, errid] = calcul_rrc2(tm1,Umrc1,Im1,'',R10,R10,C10,R20,C20);

[Rsid, R1id, C1id, R2id, C2id, rsquareid, mverrid] = calcul_rrc2_fit(tm1,Umrc1,Im1,'',R10,C10,R20,C20);
...
r2=[r2 R2id];
c2=[c2 C2id];
rsquare=[rsquare rsquareid];
mverr=[mverr mverrid];

%Us_rsr1c1
this_U_sim= rrc2_output(datetime_phase,current_phase,Rsid,R1id,C1id,R2id,C2id);
...
tau1ini=1;%duration_pulse/2;
tau2ini=30;%duration_pulse;
