...
%merge_struct - concatenante fields of structures.
%
% Output structure is the result of concatenating fields of S1 with fields
% of S2. Non existing fields in S2 mean keeping S1 values,
% non existing fields in S1 mean copying S2 values. S1 and S2 must
% have same size. Substructures are recursively concatenante.
%
% Usage: S = concat_struct(S1,S2)
% Inputs:
...
for ind = 1:length(S1)
    for ind2 = 1:length(fieldList2)
        if isstruct(S2(ind).(fieldList2{ind2})) && isfield(S1(ind),fieldList2{ind2})
            ...
            if isfield(S1(ind),fieldList2{ind2})
                S1(ind).(fieldList2{ind2}) = [S1(ind).(fieldList2{ind2})(:); S2(ind).(fieldList2{ind2})(:)];
            else
