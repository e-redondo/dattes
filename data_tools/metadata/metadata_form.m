...
if strcmpi(answer,'Y')
    ...
    % metadata.chamber.min_temperature = -20;%degC
    prompt = 'chamber minimal operating temperature in degC (e.g. -20)? ';
    answer = input(prompt,'s');
    answer = str2num(answer);
    if ~isempty(answer)
        metadata.chamber.min_temperature = answer;
    end
    % metadata.chamber.max_temperature = 60;%degC
    prompt = 'chamber maximal operating temperature in degC (e.g. 60)? ';
    answer = input(prompt,'s');
    answer = str2num(answer);
    if ~isempty(answer)
        metadata.chamber.max_temperature = answer;
    end
end
%% 4. other info
